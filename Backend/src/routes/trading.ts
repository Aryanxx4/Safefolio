...
router.post("/order", ensureAuthenticated, async (req: Request, res: Response) => {
  try {
    ...
    if (mode === "historical") {
      ...
      if (!histPrice) {
        ...
        const earliestResult = await pool.query(
          `SELECT MIN(price_date) as earliest_date FROM public.equity_prices WHERE stock_symbol = $1`,
          [symbol]
        );
    ...
    if (side === "SELL") {
      const pos = await pool.query(
        `SELECT quantity FROM positions WHERE user_id = $1 AND simulation_id = $2 AND symbol = $3`,
        [userId, simId, symbol]
      );
    ...
    try {
      // Record transaction
      await pool.query(
        `INSERT INTO transactions (user_id, simulation_id, symbol, side, quantity, price)
         VALUES ($1, $2, $3, $4, $5, $6)`,
        [userId, simId, symbol, side, quantity, price]
      );
      ...
      if (side === "BUY") {
        ...
        if (existing.rows.length > 0) {
          ...
          await pool.query(
            `UPDATE positions SET quantity = $1, average_price = $2, updated_at = NOW()
             WHERE user_id = $3 AND simulation_id = $4 AND symbol = $5`,
            [newQty, newAvg, userId, simId, symbol]
          );
        } else {
          await pool.query(
            `INSERT INTO positions (user_id, simulation_id, symbol, quantity, average_price)
             VALUES ($1, $2, $3, $4, $5)`,
            [userId, simId, symbol, quantity, price]
          );
        }
      } else {
        // SELL: Add cash
        ...
        await pool.query(
          `UPDATE positions SET quantity = quantity - $1, updated_at = NOW()
           WHERE user_id = $2 AND simulation_id = $3 AND symbol = $4`,
          [quantity, userId, simId, symbol]
        );

        // Remove position if quantity becomes 0
        await pool.query(
          `DELETE FROM positions WHERE quantity <= 0 AND user_id = $1 AND simulation_id = $2 AND symbol = $3`,
          [userId, simId, symbol]
        );
      }
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE simulations s
       ...
       RETURNING s."current_date", s.start_date, s.end_date`,
      [days.toString(), simId]
    );
...
router.get("/simulation", ensureAuthenticated, async (req: Request, res: Response) => {
  try {
    const user = req.user as AppUser;
    const userId = user.id;
    const { mode } = req.query; // Get mode from query: 'historical' or 'realtime'
    const simMode = (mode === 'realtime' ? 'realtime' : 'historical') as 'historical' | 'realtime';
    const simId = await getOrCreateSimulation(userId, simMode);

    const result = await pool.query(
      `SELECT id, name, "current_date", start_date, end_date, created_at, updated_at
...
router.get("/historical/:symbol", ensureAuthenticated, async (req: Request, res: Response) => {
  try {
    ...
    const sim = await pool.query(`SELECT "current_date" FROM simulations WHERE id = $1`, [simId]);
    ...
    const result = await pool.query(query, [symbol, currentDate]);
...
router.get("/symbols", ensureAuthenticated, async (req: Request, res: Response) => {
  try {
    const result = await pool.query(
...
router.get("/price/:symbol", ensureAuthenticated, async (req: Request, res: Response) => {
  try {
    const { symbol } = req.params;
    ...
    if (simMode === 'realtime') {
      ...
      const earliestResult = await pool.query(
        `SELECT close, price_date FROM public.equity_prices
         WHERE stock_symbol = $1
         ORDER BY price_date ASC
         LIMIT 1`,
        [symbol]
...
// Get real-time quote
router.get("/realtime/:symbol", ensureAuthenticated, async (req: Request, res: Response) => {
  try {
    const { symbol } = req.params;
    const quote = await getQuote(symbol);
