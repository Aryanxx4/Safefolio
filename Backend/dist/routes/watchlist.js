...
router.get("/", auth_1.ensureAuthenticated, async (req, res) => {
    try {
        const user = req.user;
        ...
        const watchlistResult = await pool_1.pool.query(`SELECT symbol, created_at FROM watchlist WHERE user_id = $1 ORDER BY created_at DESC`, [userId]);
        const watchlistItems = watchlistResult.rows;
        // Get prices for each symbol
        const itemsWithPrices = await Promise.all(watchlistItems.map(async (item) => {
            let price = null;
            let change = null;
            ...
            if (priceMode === 'realtime') {
                ...
                const quote = await (0, finnhub_1.getQuote)(item.symbol);
                if (quote && quote.c > 0) {
                    price = quote.c;
...
router.post("/add", auth_1.ensureAuthenticated, async (req, res) => {
    try {
        const user = req.user;
        const userId = user.id;
        ...
        const symbolCheck = await pool_1.pool.query(`SELECT DISTINCT stock_symbol FROM public.equity_prices WHERE stock_symbol = $1 LIMIT 1`, [symbol.toUpperCase()]);
        if (symbolCheck.rows.length === 0) {
            return res.status(404).json({ error: `Symbol ${symbol} not found in database` });
        }
        // Add to watchlist
        const result = await pool_1.pool.query(`INSERT INTO watchlist (user_id, symbol)
       VALUES ($1, $2)
       ON CONFLICT (user_id, symbol) DO NOTHING
       RETURNING id, symbol, created_at`, [userId, symbol.toUpperCase()]);
        if (result.rows.length === 0) {
...
router.delete("/remove/:symbol", auth_1.ensureAuthenticated, async (req, res) => {
    try {
        const user = req.user;
        const userId = user.id;
        const { symbol } = req.params;
        const result = await pool_1.pool.query(`DELETE FROM watchlist WHERE user_id = $1 AND symbol = $2 RETURNING symbol`, [userId, symbol.toUpperCase()]);
        if (result.rows.length === 0) {
